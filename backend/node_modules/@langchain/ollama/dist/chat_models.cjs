	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		if (config?.method === void 0 || config?.method === "jsonSchema") {
			const outputSchemaIsZod = (0, __langchain_core_utils_types.isInteropZodSchema)(outputSchema);
			const jsonSchema = outputSchemaIsZod ? (0, __langchain_core_utils_json_schema.toJsonSchema)(outputSchema) : outputSchema;
			...
			const llm = this.bindTools([{
				...
				ls_structured_output_format: {
					...
					schema: (0, __langchain_core_utils_json_schema.toJsonSchema)(outputSchema)
				}
			});
			/**
			* Create a parser that handles both tool calls and JSON content
			*/
			const outputParser = __langchain_core_runnables.RunnableLambda.from(async (input) => {
				/**
				* Ensure input is an AI message (either AIMessage or AIMessageChunk)
				*/
				if (!__langchain_core_messages.AIMessage.isInstance(input) && !__langchain_core_messages.AIMessageChunk.isInstance(input)) throw new Error("Input is not an AIMessage or AIMessageChunk.");
				/**
				...
				*/
				if (input.tool_calls && input.tool_calls.length > 0) {
					...
					if (toolCall && toolCall.args) {
						...
						*/
						if (outputSchemaIsZod) return await (0, __langchain_core_utils_types.interopParseAsync)(outputSchema, toolCall.args);
				...
				*/
				if (outputSchemaIsZod) {
					const zodParser = __langchain_core_output_parsers.StructuredOutputParser.fromZodSchema(outputSchema);
					return await zodParser.parse(content);
				} else {
					const jsonParser = new __langchain_core_output_parsers.JsonOutputParser();
